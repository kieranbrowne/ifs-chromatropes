...
vec3 color(vec2 pt, mat2 rot) {
  ...
  colour = mix(colour,vec3(.8,.4,.0), vec3(smoothstep(0.00,1.800,.8+sin(ngon(kal(kal(rot*pt,9)+vec2(0.9,.1),9), vec2(.3,.1), 8)*90.)*1.5)));
  colour = mix(colour,vec3(.0,0.5,.9), vec3(smoothstep(0.00,1.800,.8+cos(ngon(kal(kal(rot*pt,9)+vec2(0.9,.1),9), vec2(.3,.1),8)*90.)*1.5)));
...
void main(void) {
  ...
  vec3 light = vec3(1.);
  // slide 1
  light = occlude(light,color(uv*rotate( iGlobalTime/5.4), rotate(-length(uv) )));
  //colour = min(colour, );
  // slide 2
  light = occlude(light,color(uv*rotate(-iGlobalTime/5.4), rotate(-length(uv/1000.))));
