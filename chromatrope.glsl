...
vec3 color(vec2 pt) {
  ...
  colour = mix(colour,vec3(.9,0.4,.0), vec3(smoothstep(1.84,.000,.8+sin(ngon(pt, vec2(0.,0.4), 3)*90.)*1.2)));
  colour = mix(colour,vec3(.4,0.9,.9), vec3(smoothstep(.000,1.84,.8+sin(ngon(pt, vec2(0.,0.5), 3)*80.)*1.2)));
  // colour = mix(colour,vec3(.0,.2,.6), smoothstep(0.36,0.35,abs(.1-circle(kal(pt, 8.), vec2(0.7,0.8), .9))));
  // colour = mix(colour,vec3(.0,0.2,.6), vec3(smoothstep(.000,1.84,-.9+sin(ngon(pt, vec2(0.,0.9), 4)*30.)*3.2)));
  // colour = mix(colour,vec3(.1,.2,0.6), smoothstep(0.06,0.05,abs(length(pt)*2.-circle(kal(pt, 8.), vec2(.1,0.4),.1))));
  //colour = mix(colour,vec3(1.,.0,1.), smoothstep(0.06,0.05,abs(.81-circle(kal(pt, 17.), vec2(0.2,0.3), .2))));
...
void main(void) {
  ...
  vec3 light = vec3(1.);
  // slide 1
  light = occlude(light,color(uv*rotate(iGlobalTime/4.)));
  //colour = min(colour, );
  // slide 2
  light = occlude(light,color(uv*rotate(-iGlobalTime/4.)));
