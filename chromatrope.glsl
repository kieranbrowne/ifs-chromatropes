...
vec3 color(vec2 pt) {
  ...
  colour = mix(colour,vec3(.0,0.2,.0), vec3(smoothstep(0.00,1.800,.8+sin(ngon(kal(kal(pt,0)+vec2(0.3,0.2),3), vec2(0.3,0.9), 3)*90.)*1.9)));
  colour = mix(colour,vec3(.8,0.6,.4), vec3(smoothstep(0.00,1.800,.8+cos(ngon(kal(kal(pt,0)+vec2(0.3,0.2),3), vec2(0.3,0.9), 3)*90.)*1.2)));
